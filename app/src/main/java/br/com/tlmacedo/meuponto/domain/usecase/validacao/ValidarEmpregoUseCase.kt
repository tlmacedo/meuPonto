...
class ValidarEmpregoUseCase @Inject constructor(
    ...
    sealed class ErroValidacao(val mensagem: String, val codigo: String) {
        ...
        data object NomeMuitoCurto :
            ErroValidacao("Nome deve ter pelo menos 2 caracteres", "NOME_CURTO")

        data object NomeMuitoLongo :
            ErroValidacao("Nome deve ter no máximo 100 caracteres", "NOME_LONGO")

        data class NomeDuplicado(val nome: String) :
            ErroValidacao("Já existe um emprego com o nome '$nome'", "NOME_DUPLICADO")
    }
